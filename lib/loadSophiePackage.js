...
module.exports = function loadSophiePackage(pack, savePath) {
  ...
  return repo.listReleases()
    ...
    .catch(e => {
      debug('failed to load from github, check if pack exists on disk');
      try {
        if (fs.readdirSync(savePath).length > 1) {
          debug('pack exists on disk');
          return Promise.resolve(pack);
        } else {
          throw new Error();
        }
      } catch (e) {
        debug('pack does not exist on disk. this will fail bad');
